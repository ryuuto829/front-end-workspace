...
function dominantDirection(text) {
  ...
  // Create two lists with ranges
  let listLTR = SCRIPTS.filter(s => s.direction == "ltr").map(s => s.ranges);
  let listRTL = SCRIPTS.filter(s => s.direction == "rtl").map(s => s.ranges);

  let ltr = foo(listLTR);
  let rtl = foo(listRTL);

  // Counter for letters in given list
  function foo(script) {
    let count = 0
    for (let char of text) {

      for (let item of script) {
        item.forEach(([from, to]) => {
          if (from <= char.codePointAt(0) && char.codePointAt(0) < to) count += 1;
        })
      }
    }
    return count;
